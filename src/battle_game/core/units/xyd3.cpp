...
Xyd::Xyd(GameCore *game_core, uint32_t id, uint32_t player_id)
...
void Xyd::TankMove(float move_speed, float rotate_angular_speed) {
...
bool Xyd::IsHit(glm::vec2 position) const {
...
const char *Xyd::UnitName() const {
  return "xyd3";
}

const char *Xyd::Author() const {
  return "Xu-Yudong";
}
}  // namespace battle_game::unit
